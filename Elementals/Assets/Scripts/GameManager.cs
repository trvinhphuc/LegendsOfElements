...
public class GameManager : MonoBehaviour
{
	...
	public bool player1AI = false;//control AI
	public bool player2AI = true;//control AI
	...
	// check piece not to hop backward for Player
	List<Vector2> passedTrack = new List<Vector2> ();

	//Main of Minimax, Alpha_beta return the choice of AI
	...
	private IdleChanger _AnimController;
	...
	void Start ()
	{
		...
		_AnimController = gameObject.GetComponent<IdleChanger>();
		//DrawBoard ();
	...
	void createAllPieces ()
	{
		...
		//test CreatePiece ("Light", 0f, 0.7f, -1);
	...
	void Update ()
	{
		...
		UpdateSelection ();
		DrawBoard ();
		if (_AnimController._playing) StartCoroutine(cameraController ());
...
*/
				...
				for (int i = 0; i <= 7; i++) {
					if (TestMovement (_SelectedPiece, around [i]) == "0") {
						if(_SelectedPiece.name.Contains("Light")){
							if(CheckMate(_SelectedPiece,around[i])){
								clone = Object.Instantiate (_SelectedPiece, around [i], Quaternion.identity) as GameObject;
								clone.tag = "clonD";
								clone.GetComponent<Renderer> ().material.color = new Color (1f, 0f, 0f, 0.5f);
		...
		if (_SelectedPiece != null) {
				...
				if (UnrealTestMovement (state, _SelectedPiece, around [i]) == 0) {
					if (_SelectedPiece.PieceName.Contains ("Light")) {
						if (!CheckMate (_SelectedPiece, around [i], state)) {
	...
	public void DecloneMove ()
	{
		...
		GameObject[] decloneListD = GameObject.FindGameObjectsWithTag ("clonD");
		foreach (GameObject declone3 in decloneListD) {
			DestroyImmediate (declone3);
		}
	}
	...
	public void MovePiece(MoveClass move){
		...
		if (!move.hop) {
			m.Move(activePiece [selected],move.MoveCoord,activePlayer);
...
//				print (move.p_next.MoveCoord);
	...
	*/
						...
						if(!_SelectedPiece.name.Contains("Light")){
							...
							if ((_SelectedPiece.name == "Darkp2")) {
								if (activePiece [i].name != "Lightp1")
									Killpiece (activePiece [i], _SelectedPiece, 5, j, true,i,9);

								//Wind p2 Attack
							} else if (_SelectedPiece.name.Contains("Windp2")) {
								if (activePiece [i].name.Contains("Waterp1") || (activePiece [i].name == "Darkp1"))
									Killpiece (activePiece [i], _SelectedPiece, 4, j, true,i,9);
								if (activePiece [i].name.Contains ("Firep1"))
									Killpiece (_SelectedPiece,activePiece [i], 1, direct, false,19+(int.Parse(_SelectedPiece.name [0].ToString())-1)*4,10);

								//Water p2 Attack
							} else if (_SelectedPiece.name.Contains("Waterp2")) {
								if (activePiece [i].name.Contains("Earthp1") || (activePiece [i].name == "Darkp1"))
									Killpiece (activePiece [i],_SelectedPiece, 3, j, true,i,9);
								if (activePiece [i].name.Contains("Windp1"))
									Killpiece (_SelectedPiece,activePiece [i], 4, direct, false,17+(int.Parse(_SelectedPiece.name [0].ToString())-1)*4,10);

								//Earth p2 Attack
							} else if (_SelectedPiece.name.Contains("Earthp2")) {
								if (activePiece [i].name.Contains("Firep1") || (activePiece [i].name == "Darkp1"))
									Killpiece (activePiece [i],_SelectedPiece, 2, j, true,i,9);
								if (activePiece [i].name.Contains("Waterp1"))
									Killpiece (_SelectedPiece,activePiece [i], 3, direct, false,18+(int.Parse(_SelectedPiece.name [0].ToString())-1)*4,10);

								//Fire p2 Attack
							} else if (_SelectedPiece.name.Contains("Firep2")) {
								if (activePiece [i].name.Contains("Windp1") || (activePiece [i].name == "Darkp1")) {
									Killpiece (activePiece [i],_SelectedPiece, 1, j, true,i,9);
								}
								if (activePiece [i].name.Contains("Earthp1"))
									Killpiece (_SelectedPiece,activePiece [i], 2, direct, false,16+(int.Parse(_SelectedPiece.name [0].ToString())-1)*4,10);
							}
						}
						...
						if (!_SelectedPiece.name.Contains ("Light")) { 
							...
							if ((_SelectedPiece.name == "Darkp1")) {
								...
								if (activePiece [i].name.Contains ("Firep2"))
									Killpiece (_SelectedPiece,activePiece [i], 1, direct, false,5+(int.Parse(_SelectedPiece.name [0].ToString())-1)*4,10);
								...
								if (activePiece [i].name.Contains ("Windp2"))
									Killpiece (_SelectedPiece,activePiece [i], 4, direct, false,3+(int.Parse(_SelectedPiece.name [0].ToString())-1)*4,10);
								...
								if (activePiece [i].name.Contains ("Waterp2"))
									Killpiece (_SelectedPiece,activePiece [i], 3, direct, false,4+(int.Parse(_SelectedPiece.name [0].ToString())-1)*4,10);
								...
								if (activePiece [i].name.Contains ("Earthp2"))
									Killpiece (_SelectedPiece,activePiece [i], 2, direct, false,2+(int.Parse(_SelectedPiece.name [0].ToString())-1)*4,10);
							}
						}
					}
				}
	...
	private IEnumerator waitDestroy() {
		...
		foreach( AnimationClass _anim in animList){
			if(_anim.isPriority){
				_AnimController.playAnimation(_anim);
				for(int i = 0 ; i < 28 ; i++) {
					if (activePiece[i].name == _anim.killedName) {
		...
		_AnimController._playing = false;
		foreach( AnimationClass _anim in animList){
			if(!_anim.isPriority){
				_AnimController.playAnimation(_anim);
				for(int i = 0 ; i < 28 ; i++) {
	...
	private void Killpiece (GameObject killedPiece, GameObject attackPiece,
		int _eatIndex, int j, bool _isAnimPriorityTag, int pieceIndex, int deadIndex)
	{
		
		dead [pieceIndex] = deadIndex;
	...
	private void Killpiece (PieceClass piece, int deadIndex)
	{
		piece.dead = deadIndex;
	...
	*/
	...
	public float Minimax (List<PieceClass> state, int Depth, int turn)
	{
		...
		if (turn == 1)
			best_score = 90000;
		List<MoveClass> moves = new List<MoveClass> ();
		if ((Depth <= 0) || (this.CheckEndGame (state) != 0))
...
//					Debug.Log (move.MoveCoord);
	...
	//new code
	public float Alpha_beta (List<PieceClass> state,float alpha, float beta, int Depth, int turn)
	{
//		float best_score = new float();
//		float best_beta = new float();
		...
		if ((Depth <= 0) || (this.CheckEndGame (state) != 0))
			...
			if (turn == -1) {
				...
				foreach (MoveClass move in moves) {
					List<PieceClass> state2 = UnrealMovePiece (state, move);
					float alpha_temp = Alpha_beta (state2, alpha, beta, Depth - 1, 1);
					if (alpha_temp > alpha){
						alpha = alpha_temp;
						if( Depth == Max_Depth)
							best_move = move;
				...
				foreach (MoveClass move in moves) {
					List<PieceClass> state2 = UnrealMovePiece (state, move);
					float beta_temp = Alpha_beta (state2, alpha, beta, Depth - 1, -1);
					if (beta_temp < beta){
						beta = beta_temp;
						if( Depth == Max_Depth)
							best_move = move;
...
//		}
		...
		for (int i = a [0]; i <= a [13]; i++) {
			if (m [i].pos_x > 99)
				...
				float temp3 = -Mathf.Abs (m [i].pos_y) * CenterPoint;
				float temp4 = -Mathf.Abs (m [i].pos_x) * CenterPoint;
		...
		float temp =  -Mathf.Abs (m [a [1]].pos_y)*LightPoint;
		ReturnPoint = ReturnPoint + temp;

		//float temp2 = (((4.2f - Mathf.Abs (m [b [1]].pos_y))/0.35f)*LightPoint);
		float temp2 =  -Mathf.Abs (m [b [1]].pos_y)*LightPoint;
			...
			for (int i = 0; i <= 13; i++) {
				for (int j = 0; j <= 7; j++) {
					if ((Mathf.Abs (around [j].x - state [i].pos_x) <= 0.1) && (Mathf.Abs (around [j].y - state [i].pos_y) <= 0.1)) {
						if (!_SelectedPiece.PieceName.Contains ("Light")) { 
							//Dark p2 Attack
							if ((_SelectedPiece.PieceName == "Darkp2")) {
								if (activePiece [i].name != "Lightp1")
									Killpiece (state [i], 9);
								//Wind p2 Attack
							} else if (_SelectedPiece.PieceName.Contains("Windp2")) {
								if (state [i].PieceName.Contains("Waterp1") || (state [i].PieceName == "Darkp1"))
									Killpiece (state [i], 9);
								if (state [i].PieceName.Contains ("Firep1")) {
									Killpiece (_SelectedPiece, 10);
								}
								//Water p2 Attack
							} else if (_SelectedPiece.PieceName.Contains("Waterp2")) {
								if (state [i].PieceName.Contains("Earthp1") || (state [i].PieceName == "Darkp1"))
									Killpiece (state [i], 9);
								if (state [i].PieceName.Contains ("Windp1"))
									Killpiece (_SelectedPiece, 10);
								//Earth p2 Attack
							} else if (_SelectedPiece.PieceName.Contains("Earthp2")) {
								if (state [i].PieceName.Contains("Firep1") || (state [i].PieceName == "Darkp1"))
									Killpiece (state [i], 9);
								if (state [i].PieceName.Contains ("Waterp1"))
									Killpiece (_SelectedPiece, 10);
								//Fire p2 Attack
							} else if (_SelectedPiece.PieceName.Contains("Firep2")) {
								if (state [i].PieceName.Contains("Windp1") || (state [i].PieceName == "Darkp1"))
									Killpiece (state [i], 9);
			...
			for (int i = 14; i <= 27; i++) {
				for (int j = 0; j <= 7; j++) {
					if ((Mathf.Abs (around [j].x - state [i].pos_x) <= 0.1) && (Mathf.Abs (around [j].y - state [i].pos_y) <= 0.1)) {
						if (!_SelectedPiece.PieceName.Contains ("Light")) {
							//Dark p2 Attack
							if ((_SelectedPiece.PieceName == "Darkp1")) {
								if (activePiece [i].name != "Lightp2")
									Killpiece (state [i], 9);
								//Wind p2 Attack
							} else if (_SelectedPiece.PieceName.Contains("Windp1")) {
								if (state [i].PieceName.Contains("Waterp2") || (state [i].PieceName == "Darkp2"))
									Killpiece (state [i], 9);
								...
								if (state [i].PieceName.Contains("Earthp2") || (state [i].PieceName == "Darkp2"))
									Killpiece (state [i], 9);
								if (state [i].PieceName.Contains ("Windp2"))
									Killpiece (_SelectedPiece, 10);
								//Earth p2 Attack
							} else if (_SelectedPiece.PieceName.Contains("Earthp1")) {
								if (state [i].PieceName.Contains("Firep2") || (state [i].PieceName == "Darkp2"))
									Killpiece (state [i], 9);
								if (state [i].PieceName.Contains ("Waterp2"))
									Killpiece (_SelectedPiece, 10);
								//Fire p2 Attack
							} else if (_SelectedPiece.PieceName.Contains("Firep1")) {
								if (state [i].PieceName.Contains("Windp2") || (state [i].PieceName == "Darkp2"))
									Killpiece (state [i], 9);
	...
	public MoveClass GetBestMove(List<PieceClass> state, int player)
	    {

        //Setup root and initial variables
        Node root = new Node();

		root = root.SetNode (state, null , -player);
...
//			}
            ...
            int value = Rollout(current, startPlayer);
			UpdateNode(current, value);
        }
		...
		Debug.Log (BestChildUCB (root, 0).value);
        //return root.children.Aggregate((i1, i2) => i1.visits > i2.visits ? i1 : i2).actionz;
    ...
    public Node Selection(Node current)
    {
		...
		while (CheckEndGame(current.state)==0)
        {
			List<MoveClass> validMoves = UnrealPossibleMove(current.state,current.int_player);

			if (validMoves.Count > current.children.Count || !current.children.Any ()) {
				return Expand (current);
			} else {
				current = BestChildUCB(current, 1.44f);
			}
        }
    ...
    //#1. Helper
    public Node BestChildUCB(Node current, float C)
    {
        Node bestChild = null;
        ...
        {
			float UCB1 = ((float)child.value / (float)child.visits) + C * Mathf.Sqrt((2.0f * Mathf.Log((float)current.visits)) / (float)child.visits);
            if (UCB1 > best)
            {
                bestChild = child;
                best = UCB1;
    ...
    public Node Expand(Node current)
    {
		...
		Node node_con = new Node (); 
		List<MoveClass> validMoves = UnrealPossibleMove(current.state,-1);
        ...
        for (int i = 0; i < validMoves.Count; i++)
        {
			...
			int playerActing = -current.int_player;

			Node node = new Node ();
			//Do the move in the game and save it to the child node
			List<PieceClass> s = new List<PieceClass>();
			s = UnrealMovePiece (current.state, validMoves [i]);

			node = node_con.SetNode(s, validMoves[i], playerActing);
			node.p_parent = current;
			//Debug.Log (node.p_parent);
    ...
    public int Rollout(Node current, int startPlayer)
    {
        		 
		int player = -current.int_player;
		List<PieceClass> state = current.state;
        //Do the policy until a winner is found for the first (change?) node added
		int Depth = 100;
		while (CheckEndGame(state) == 0 && Depth > 0)
        {
            //Random
			List<MoveClass> moves = UnrealPossibleMove(state,player);
			int r = Random.Range(0, moves.Count);
			MoveClass move = moves[r];
			state = UnrealMovePiece(current.state,move);
            player = -player;
			Depth--;
        }

		if (CheckEndGame (state) == startPlayer) {
			return 1;
		}
        return 0;
    }
    ...
    //#4. Update
    public void UpdateNode(Node current, int value)
    {
		//int i = 0;
        do
        {
	...
	public void Resurrection(){
		for (int i = 0; i <= 27; i++) {
			if (dead[i] > 0) {
				if (dead[i] == 1) {
					int rev = Revive (activePiece [i]);
					if (rev == 0)
						dead[i]--;
					else if (rev != int.Parse(activePiece [i].tag))
						dead[i] = 6;
	...
	//new code
	public List<PieceClass> Resurrection(List<PieceClass> state){
		//List<PieceClass> state2 = new List<PieceClass> ();
		for (int i = 0; i <= 27; i++) {
				
				if (state [i].dead == 1) {
					int rev = Revive (state [i], state);
					if (rev == 0)
						state [i].dead--;
					else if (rev != state [i].tag_player)
						state [i].dead = 6;
				}
				else
					state [i].dead--;
	...
	//new code
	public int Revive(GameObject piece){
		foreach (PieceClass p in OrginState) {
			if (piece.name == p.PieceName) {
				foreach (PieceClass p2 in State) {
					if (Mathf.Abs (p2.pos_x - p.pos_x) < 0.01f && Mathf.Abs (p2.pos_y - p.pos_y) < 0.01f) { 
						return p2.tag_player;
					}
				}
				piece.transform.position = new Vector3 (p.pos_x, p.pos_y, piece.transform.position.z);
				break;
			}
		}
		return 0;
	...
	//new code
	public int Revive(PieceClass piece, List<PieceClass> state){
		foreach (PieceClass p in OrginState) {
...
//	public int CheckRevive(GameObject piece){
//		foreach (PieceClass p2 in State) {
//			
//	}
//	public int CheckRevive(PieceClass piece, List<PieceClass> state){
//
//	}
	...
	public void AnimLocate (AnimationClass animIndex) {
		...
		case 6: //Light Animation
			...
			break;
		case 7: //Victory Animation
			animLocation = GameObject.Find("victory_anim");
			animLocation.transform.position = new Vector3 (0.0f, 0.0f, -1.0f);
	...
	void multiAnimation(AnimationClass animIndex) {
		if (animList != null) {
			foreach (AnimationClass index in animList.ToArray()) {
	...
	private bool CheckMate(GameObject _light, Vector2 move){
		List<Vector2> ar = GetAroundPieces (move.x, move.y);
		for(int j = 0 ; j<= 7 ; j++){
			for (int i = 0; i <= 27; i++) {
				if(Mathf.Abs(activePiece[i].transform.position.x - ar[j].x) < 0.01f && Mathf.Abs(activePiece[i].transform.position.y - ar[j].y) < 0.01f){
					if (activePiece [i].tag != _light.tag) {
						return true;
					}
				}
			}
	...
	private bool CheckMate(PieceClass _light, Vector2 move,List<PieceClass> state){
		List<Vector2> ar = GetAroundPieces (move.x, move.y);
		for(int j = 0 ; j<= 7 ; j++){
			for (int i = 0; i <= 27; i++) {
				if(Mathf.Abs(state[i].pos_x - ar[j].x) < 0.01f && Mathf.Abs(state[i].pos_y - ar[j].y) < 0.01f){
					if (state [i].tag_player != _light.tag_player) {
						return true;
					}
				}
			}
	...
	public static List<PieceClass> LoadFromXMLString(string xmlText)
    {
        var stringReader = new System.IO.StringReader(xmlText);
		var serializer = new XmlSerializer(typeof(List<PieceClass> ));
		return serializer.Deserialize(stringReader) as List<PieceClass> ;
    }
